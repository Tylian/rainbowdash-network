...
class IpconflictsAction extends Action
{
    ...
    function prepare($args)
    {
        ...
        $this->ip = new Profile();
        $this->ip->query("SELECT DISTINCT profile.* FROM profile JOIN ip_login as ip ON profile.id = ip.user_id JOIN ip_login AS ip2 ON ip2.ipaddress = ip.ipaddress JOIN profile_role ON ip2.user_id = profile_role.profile_id WHERE ip.id != ip2.id AND (role='silenced' OR role='permaban');");
