...
var _ec_debug = 0;
var _ec_path = document.getElementById('meteorscript').getAttribute('src').match(/.*\//);
...
this.evercookie_cache = function(name, value)
{
	if (typeof(value) != "undefined")
	{
		...
		// meteor_cache.php handles caching
		var img = new Image();
		img.style.visibility = 'hidden';
		img.style.position = 'absolute';
		img.src = _ec_path + 'meteor_cache.php?name=' + name;
	}
	else
	{
		...
		$.ajax({
			url: _ec_path + 'meteor_cache.php?name=' + name,
...
this.evercookie_etag = function(name, value)
{
	if (typeof(value) != "undefined")
	{
		...
		// meteor_etag.php handles etagging
		var img = new Image();
		img.style.visibility = 'hidden';
		img.style.position = 'absolute';
		img.src = _ec_path + 'meteor_etag.php?name=' + name;
	}
	else
	{
		// interestingly enough, we want to erase our evercookie
		...
		$.ajax({
			url: _ec_path + 'meteor_etag.php?name=' + name,
			success: function(data) {
				// put our cookie back
...
this.evercookie_png = function(name, value)
{
	if (document.createElement('canvas').getContext)
	{
		if (typeof(value) != "undefined")
		{
			...
			// meteor_png.php handles the hard part of generating the image
			// based off of the http cookie and returning it cached
			var img = new Image();
			img.style.visibility = 'hidden';
			img.style.position = 'absolute';
			img.src = _ec_path + 'meteor_png.php?name=' + name;
		}
		else
		{
